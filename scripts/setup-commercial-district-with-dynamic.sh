...
# 商圈活化儀表板自動化部署腳本 (含動態地圖保護)
# 作者: Taipei City Dashboard Team
# 版本: 2.0 - 增強版本 (保護動態地圖功能)
# 目的：執行基礎商圈設置後自動恢復動態地圖功能

set -e  # 任何命令失敗就退出

echo "🚀 開始部署商圈活化儀表板 (含動態地圖保護)..."
...
check_scripts() {
    ...
    if [ ! -f "scripts/setup-commercial-district.sh" ]; then
        print_error "找不到基礎設置腳本: scripts/setup-commercial-district.sh"
        exit 1
    fi
...
# 執行基礎商圈設置
execute_basic_setup() {
    print_status "步驟 1/2：執行基礎商圈活化設置..."
    echo "=================================================="
    
    ./scripts/setup-commercial-district.sh
    
    if [ $? -eq 0 ]; then
        print_success "基礎商圈活化設置完成"
    else
        print_error "基礎設置失敗"
        exit 1
    fi
...
final_verification() {
    ...
    # 檢查台北組件是否有動態地圖配置
    TAIPEI_MAP_ID=$(docker exec postgres-manager psql -U postgres -d dashboardmanager -t -c "SELECT map_config_ids[1] FROM public.query_charts WHERE index = 'commercial_district_density' AND city = 'taipei';" 2>/dev/null | tr -d ' ')
    
    if [ ! -z "$TAIPEI_MAP_ID" ] && [ "$TAIPEI_MAP_ID" != "NULL" ]; then
        print_success "✅ 台北市集動態地圖功能已就緒"
    else
        print_warning "⚠️ 台北市集動態地圖配置可能有問題"
    fi
    
    # 檢查 GeoJSON 檔案
    if [ -f "Taipei-City-Dashboard-FE/public/mapData/market_events_taipei.geojson" ]; then
        FEATURE_COUNT=$(jq '.features | length' Taipei-City-Dashboard-FE/public/mapData/market_events_taipei.geojson 2>/dev/null || echo "0")
        if [ "$FEATURE_COUNT" -gt "0" ]; then
            print_success "✅ 台北市集 GeoJSON 檔案正常 ($FEATURE_COUNT 個活動)"
        else
...
show_completion_message() {
    ...
    echo ""
    echo "📊 包含組件:"
    echo "  • 商圈人流分析: 顯示人流變化趨勢"
    echo "  • 市集活動分佈: 動態顏色地圖視覺化"
    echo "  • 行政區圖表: 台北市12區商圈統計"
    echo ""
    echo "🌐 如何查看:"
    ...
    echo "  • 重新執行: ./scripts/setup-commercial-district-with-dynamic.sh"
    echo "  • 單獨恢復動態功能: ./scripts/restore-dynamic-map-config.sh"
    echo ""
    print_success "🚀 部署完成！現在可以享受動態市集地圖了！"
}
