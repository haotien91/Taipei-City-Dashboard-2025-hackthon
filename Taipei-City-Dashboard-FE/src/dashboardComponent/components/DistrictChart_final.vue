...
import { loadEventData } from "../../components/taipei_event/EventLoader.js";
...
// 從CSV動態載入市集資料
const districtEvents = ref({});

// 載入真實市集資料
onMounted(async () => {
	const events = await loadEventData();
	districtEvents.value = events;
	console.log('載入的真實市集資料:', events);
}); };
...
function handleDataSelection(index) {
	// 獲取點擊的行政區名稱
	const districtName = districts[index];
	
	// 獲取該行政區的活動資料
	const events = districtEvents[districtName] || [];
	
	// 顯示區域活動資訊彈跳視窗（無論有無活動都顯示）
	dialogStore.districtInfoContent = {
		districtName: districtName,
		events: events
	};
	dialogStore.showDialog('districtInfo');
